...
export default function Filmes(props) {
  ...
  async function onSubmit(event) {
    ...
    console.log(nomeFilme.target.value)
    console.log(selectedCategoriaID)
    const response = await fetch('http://127.0.0.1:8000/filmes/', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify([{nome: nomeFilme.target.value, categoria_FK: selectedCategoriaID}]),
    })
              ...
              <div className="card">
                <form onSubmit={onSubmit} className="p-fluid">
                  <div className="field gitImgContainer flex align-items-center justify-content-center border-circle">
                    <p className="text-6xl text-white">Cadastrar filmes</p>
                  </div>
                  <div className="field">
                    <span className="p-float-label">
                      <InputText
                        id="name"
                        name="name"
                        value={nomeFilme.value}
                        onChange={newText => setNomeFilme(newText)}
                        autoFocus
                      />
                  ...
                  <div className="field">
                    <span className="p-float-label">
                      <Dropdown
                        ...
                        onChange={(e) => (setSelectedCategoria(e.value), setSelectedCategoriaID(e.value.id))}
                        options={categorias}
                        optionLabel="nome"
                      />
